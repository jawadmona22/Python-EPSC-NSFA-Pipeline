...
# #Veruki Model Constants
# rate_constants = {
#     ('C0', 'C1'): 13660000 * 0.00002,
#     ('C1', 'C0'): 2093 * 0.00002,
#     ('C1', 'C2'): 6019000 * 0.00002,
#     ('C2', 'C1'): 4719 * 0.00002,
#     ('C2', 'O'): 17230 * 0.00002,   #OPEN STATE, HIGH PROB
#     ('O', 'C2'):  7468 * 0.00002,
#     ('C1', 'C3'): 421.9 * 0.00002,
#     ('C3', 'C1'): 31.15 * 0.00002,
#     ('C2', 'C4'): 855.3 * 0.00002,
#     ('C4', 'C2'): 46.65 * 0.00002,
#     ('O', 'C5'): 3.108 * 0.00002,
#     ('C5', 'O'): 0.6912 * 0.00002,  #OPEN STATE, LOW PROB
...
#
# # # #Geiger Model Constants
rate_constants = {
    ('C0', 'C1'): 1710000 * 0.00002,
    ('C1', 'C0'): 157 * 0.00002,
    ('C1', 'C2'): 3240000 * 0.00002,
    ('C2', 'C1'): 3760 * 0.00002,
    ('C2', 'O'): 14900 * 0.00002,
    ('O', 'C2'): 7468 * 0.00002,
    ('C1', 'C3'): 1530 * 0.00002,
    ...
    ('O', 'C5'): 241.5 * 0.00002,
    ('C5', 'O'): 191* 0.00002,
    ('C3', 'C4'): 611000 * 0.00002,
    ('C4', 'C3'): 2 * 0.00002,
    ('C4', 'C5'): 1590 * 0.00002,
    ('C5', 'C4'): 899000 * 0.00002
}
...
class StateMachine:
    ...
    def transition(self,agPulse): #agpulse is the value of the agonist pulse at the given index
        ...
        for (state_from, state_to), rate in rate_constants.items():
            if state_from == self.state:
                ...
                if (state_from == 'C1' and state_to == 'C2') or (state_from == 'C3' and state_to == 'C4'):
                    if agPulse != 0:
                        d_rateconstant = (1 / (3000 * 0.000001 * rate *agPulse)) * self.lotbl[int(10000 * random.uniform(0, 1))]
                    else:
                        d_rateconstant = 900000
                else:
                    d_rateconstant = (1 / (3000 * rate) )* self.lotbl[int(10000*random.uniform(0,1))] #In this paradigm higher rates mean smaller items, more likely to go this route
        ...
        # print(f"Best Transition: {best_transition}")
        ...
        return best_transition[1] #Specifies d value
...
def Agonist_Pulse():
    ...
    for i in range(10,1001):
        t = (i-9) * .00002
        agonist_pulse_array[i] = (5* (1 - math.exp(-t / 0.000000002))) * math.exp(-t / 0.00002) #5x10^8 rising phase, 5x10^4 decay phase
    ...
    agpulse_pd = pd.DataFrame(agPulseScaled)
    agpulse_pd.to_excel('agpulse_excel.xlsx')
...
def main():
    ...
    agonist_df = pd.DataFrame(agonist_pulse_array)
    agonist_df.to_excel("agonist_pulse.xlsx")
    time = np.linspace(0, 6, 800)
    ...
    for summed_iteration in tqdm(range(num_EPSCs), desc="Generating Summed Traces"):  # Add tqdm for progress bar
        channel_traces = []
        state_timearrays = []
        for channel in range(num_channels):  # Generate 200 channel traces
            # if channel > 50:
            #     iSC = iCP
            # else:
            #     iSC = iCI
            # print(f'Channel number {channel}')
            state_machine.reset() #Returns to C0
            single_channel_trace = []
            single_channel_state = []
            points_added = 0  # Counter for points added
            for i in range(800):  # That each have 800 time points
                if points_added >= 800:  # Ensure no more than 800 points are added
                    break
                else:
                    dXY = state_machine.transition(agonist_pulse_array[points_added])
                    time_in_state = math.ceil(0.5 + dXY)
                    # print(f"Spending {time_in_state} time in state {state_machine.state}")
                    for _ in range(time_in_state):
                        single_channel_state.append(state_machine.state)
                        if points_added >= 800:
                            break
                        ...
                        state_dict[state_machine.state] += 1
                        points_added += 1

            channel_traces.append(single_channel_trace)
            state_timearrays.append(single_channel_state)
    ...
    #Send time and agonist array to excel file
    state_df = pd.DataFrame(state_timearrays)
    state_df.to_excel("State_DF.xlsx")
    ...
    df.T.to_excel('summed-traces-data-risetime-debug-0s.xlsx', index=False, header=False) #transpose and save
    state_df = pd.DataFrame([state_dict])
    state_df.to_excel("state_counts.xlsx")
    print(state_df)
