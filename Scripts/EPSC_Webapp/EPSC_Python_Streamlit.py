...
def main():
    ...
    idx = 0
    #Setting parameters for matrix creation
    st.session_state.dv_dt_align = st.checkbox('dv/dt align?', key=str(idx) + "dv")
    st.session_state.peak_align = st.checkbox('Peak align?', key="str(idx)  + peakalign")
    st.session_state.midpoint_align = st.checkbox('Midpoint align', key=str(idx) + "midpoint align")
    st.session_state.start_from_peak = st.checkbox('Start from Peak?', key=str(idx) + " peak start")
    ...
    if st.session_state.uploaded_files is not None:
        for idx, uploaded_file in enumerate(st.session_state.uploaded_files):
            ...
            with st.expander(f"File: {uploaded_file.name}"):
                ...
                st.session_state.use_processed = st.checkbox('Use Processed Data?',key = str(idx) + "useprocessed",value=True)
                ...
                if st.button("Preprocess Data", key=str(idx) + "preprocess"):
                    ...
                    # if st.session_state.dv_dt_align :
                    #     min_checked_data = EPSC_preprocessing.align_dv_dt(min_checked_data,st.session_state.template)
                    # if (max_rise_time != 0):
                    ...
                    #     count_baseline = 0
                    if st.session_state.dv_dt_align:
                        print("Aligning DV/DT!")
                        alignment_processed,start_point = EPSC_preprocessing.align_dv_dt(st.session_state.EPSCs,st.session_state.template)
                        st.session_state.start_point = start_point


                    elif st.session_state.midpoint_align:
                        print("Aligning midpoint!")

                        alignment_processed,start_point = EPSC_preprocessing.align_midpoint(st.session_state.EPSCs,st.session_state.template)
                        st.session_state.start_point = start_point

                    elif st.session_state.peak_align:
                        print("Alignment at peak!")
                        alignment_processed = EPSC_preprocessing.align_peaks(st.session_state.EPSCs,peak_index)
                        st.session_state.start_point = peak_index

                    if st.session_state.start_from_peak:
                        st.session_state.start_point = peak_index
                    st.session_state.processed_data = alignment_processed
                    print(alignment_processed)
                    fig, axs = plt.subplots(1, 1)
...
def run_multipool_analysis(num_pools,display_plots=True,report_generation=False,risetime_sorting=False,risetime_file=None):
    if st.session_state.peak_index is None:
        ...
        if risetime_sorting:
            print("Sorting by rise-time!")
            if risetime_file == None:
                print("Rise Time File is not set in Run Multipool Analysis")
            else:
                risetimes = pd.read_excel(risetime_file)
                raw_sorted = EPSC_App_Connection.sort_EPSCs_by_risetimes(st.session_state.EPSCs,risetimes)
        else:
...
def run_single_pool_analysis(num_pools,display_plots=True,report_generation=False):
    if st.session_state.peak_index is None:
        ...
        means, vars = EPSC_App_Connection.one_pool_analysis(st.session_state.EPSCs,
                                                            st.session_state.peak_index, num_pools,
                                                            st.session_state.endPoint,
                                                            st.session_state.template,st.session_state.start_point)
        ...
        if display_plots == True:
            st.pyplot(fig)
            st.write("Initial Current (Parabolic) (i):", initial_slope)
            st.write("Initial Current (Linear) (i):", lin_initial_slope)
