...
def rise_times_histogram_creator(df,folder_name,plt_show = False):
    ...
    rise_times_df.to_excel('rise_times.xlsx',index=False)
    if plt_show == True:
        plt.figure()
        plt.hist(rise_times, bins=10, edgecolor='black')
        plt.xlabel('Rise Times (ms)')
        plt.title(f'Rise Times Histogram {folder_name}')
        plt.ylabel('Frequency')
        plt.show()
        plt.savefig(f'{folder_name}rise_times_histogram')
    return rise_times
...
def compare_IEM_control():
    ...
    IEM_ordered_names = []
    for sheet_name in IEM_sheet_names:
